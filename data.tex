...
\newcommand{\cvhowami}{
with about 5 years of production experience in Python development. Have an experience in \\
developing asynchronous web-projects on Python.\\
...
\newcommand{\cvexperience}{
	\begin{entrylist}
		...
		{Montenegro}
		{
            ...
            • Developed and processed ElasticSearch aggregation queries with a different complexity \\
            • Developed and processed queries for PostgreSQL database, using both psql syntax and query builder \\
            • Run services on AWS using Jenkins pipelines and checking instances states using AWS API, worked with AWS Cost Explorer \\
            • Have experience with messages brokers (RabbitMQ) \\
            • Worked with app logs setup and processing using Kafka \\
            • Performed unit testing for app functionality \\
            • Worked with Nomad and Docker, e.g. managing containers, writing Dockerfile \\
            • Used gitlab CI/CD tools during the work process \\
            • Proceed work with Scrum framework, performed code review, used linters, typing checkers, formatters \\
			\texttt{FastAPI}\slashsep\texttt{Python3}\slashsep\texttt{Tortoise}\slashsep\texttt{aiohttp}\\
			\texttt{PostgreSQL}\slashsep\texttt{RabbitMQ}\slashsep\texttt{ElasticSearch}\slashsep\texttt{Kafka}\\
			\texttt{Redis}\slashsep\texttt{AWS}\slashsep\texttt{Docker}\slashsep\texttt{Nomad}\\
